(function (Scratch) {
  ...
  var setteammlb = "";
  // NFL API SAMPLE //
  const url = "https://tank01-nfl-live-in-game-real-time-statistics-nfl.p.rapidapi.com/getNFLPlayerInfo?playerName=karl%20brooks&getStats=true";
  ...
  class Sports {
    getInfo() {
      return {
        ...
        blocks: [
          ...
          {
            blockType: "label",
            text: "Baseball/Softball/MLB (Coming Soon)",
          },
        ],
        menus: {
          FORMAT_MENU: {
    ...
    playerInfomation({ OPTION }) {
      try {
        switch (OPTION) {
          case "short name": return data[0].cbsShortName;
          case "jersey": return data[0].jerseyNum;
          case "team": return data[0].team;
          case "height": return data[0].height;
          case "age": return data[0].age;
          case "birthday": return data[0].bDay;
          case "position": return data[0].pos;
        }
      } catch (error) {
    ...
    superbowl() {
      var countDownDate = new Date("Feb 11, 2024 18:00:00").getTime();
    ...
    teamstats({ FORMAT }) {
      try {
        ...
        switch (FORMAT) {
          case "ratio": return `${datas.wins}-${datas.loss}`;
          case "abbreviation": return datas.teamAbv;
          case "streak":
